...
function drawHeader(pdf, x, y, width, height, headerText) {
    ...
    pdf.setFillColor(0, 40, 104); // Couleur bleu
    pdf.setDrawColor(0, 40, 104); // Couleur bleu
...
async function genererPDF(data) {
	...
	//Pavé Informations principales
		...
		pdf.setFontSize(12); // Taille de la police
		pdf.setTextColor(0, 40, 104); // Texte bleu
			...
			if(nameOfFirstPiece){ // Si une pièce est renseignée, on ajoute le titre et le tableau, sinon rien ne s'affiche	
				...
				pdf.autoTable({
					...
					styles: {
						...
						textColor: [0, 40, 104], // Texte bleu
						lineColor: [0, 40, 104], // Couleur des bordures (rouge en RGB)
				...
				new Compressor(file, {
					...
					success(result) {
						...
						reader.onload = () => {
							...
							pdf.setFont("helvetica", "bold");
							pdf.setFontSize(12);
							pdf.setTextColor(0, 40, 104); // Texte bleu EDF
...
function addSignatures(pdf, entreprise, representantNom, agentNom, signatures, startY) {
    ...
    pdf.setDrawColor(0, 40, 104); // Bordures bleu EDF
    pdf.setTextColor(0, 40, 104); // Texte bleu EDF
...
function addFootPage(pdf) {
    ...
    for (let i = 1; i <= pageCount; i++) {
        ...
        pdf.setFont("helvetica", "bold");
        pdf.setFontSize(10);
        pdf.setTextColor(0, 40, 104); // Texte bleu foncé
